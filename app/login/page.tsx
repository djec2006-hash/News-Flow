...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Google Login: auth error:", error)
        setError(error.message)
        setLoading(false)
      }
    } catch (err: any) {
      console.error("[v0] Google Login: unexpected error:", err)
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            >
              ...
              <span className="relative z-10 flex items-center justify-center gap-3">
                ...
                <span className="text-zinc-100 group-hover:text-white transition-colors">Continuer avec Google</span>
              </span>
            </button>
            ...
            <div className="flex items-center justify-between my-4">
              <div className="flex items-center gap-2">
                <input
                  ...
                  type="checkbox"
                  disabled={loading || !supabaseEnabled}
                  className="w-4 h-4 rounded border-zinc-600 bg-transparent text-indigo-500 accent-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:ring-offset-0 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <label htmlFor="remember" className="text-sm text-zinc-400 cursor-pointer select-none disabled:opacity-50">
                  Se souvenir de moi
                </label>
              </div>
              <Link 
